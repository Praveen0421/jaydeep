    ...
    responses.forEach((value) => {
      ...
      tableHeader.forEach((element) => {
        ...
        if (element.action !== "ACTION") {
          ...
          editBtn.setAttribute("title", "Edit Data");
          ...
          deleteBtn.setAttribute("title", "Delete Data");
...
const toFilter = document.querySelector('#filterStatusBtn');
toFilter.addEventListener('change', () => {
  let status = toFilter.value;
  filterData(status);
})
...
function filterData(status) {
  ...
  var tbaleRow = table.querySelectorAll('tr')
  for (let iterator of tbaleRow) {
    let row = iterator;
    var column = row.querySelector('#status');
    console.log(row);
    if (column) {
      let rowStatus = column.textContent;
      if (rowStatus !== status) {
